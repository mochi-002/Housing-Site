 ...
 */
const requestApartment = asyncHandler(async (req, res) => {
    // 1. check apartment
    ...
    if (apartment.owner.equals(req.user._id)) {
        sendError(res, {
            message: "You can't request your own apartment",
            statusCode: 403,
        });
        return;
    }
    ...
    if (apartment.status !== 'available') {
        sendError(res, {
            message: `Apartment isn't available for registration`,
            statusCode: 403,
        });
        return;
    }
...
const deleteRequest = asyncHandler(async (req, res) => {
    ...
    if (!request) {
        sendError(res, { message: 'Request not found', statusCode: 404 });
        return;
    }
...
const decline = asyncHandler(async (req, res) => {
    const request = await InterestRequest.findById(req.params.id);
    if (!request) {
        sendError(res, { message: 'Interest request not found', statusCode: 404 });
        return;
    }
    const listing = await Listing.findById(request.listing);
    if (!listing) {
...
export { requestApartment, deleteRequest, accept as acceptRequest, decline as declineRequest, };
